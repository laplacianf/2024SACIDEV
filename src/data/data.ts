...
export const problemInfo: IProblem[] = [
    ...
    {
        problem: "SAC이 너무 기대되는 길쭉이는 SAC이 시작하기 전부터 매일 SAC 시작으로부터 몇일이 남았는지를 떠올리고 있다. 길쭉이는 기억력이 좋지 않으므로, 특정 시간이 지난 이후 길쭉이가 지금까지 떠올린 모든 수를 차례로 출력하는 프로그램을 작성하자.\n\n첫 번째 입력으로 SAC 시작으로부터 지난 시간이 주어진다.",
        example: [
            {
                input: [1],
                output: "1"
    ...
    {
        problem: "지난 SAF때 여러분의 도움으로 길쭉이는 어떤 자연수 N에 대해서 N 이하의 가장 큰 소수를 구할 수 있었다. ",
        example: [
            {
                input: [3, 2],
                output: "5"
